...
namespace QuickCross
{
	public class ViewBase : UIViewController, ViewDataBindings.ViewExtensionPoints
	{
		...
		{
			if (!(sender is BaseNavigator) && viewModel != null) {
				string commandName = segue.Identifier;
				if (viewModel.ExecuteCommand (commandName, GetCommandParameter (commandName)))
					return;
			} 
		...
		public override void ViewWillAppear (bool animated)
		{
			...
			//Tuan- Check ViewModel to resolve Bug: ViewWillAppear is called 2 twice 
			//		when call function imagePicker.DismissViewController()
			if (viewModel == null)
				return;
			EnsureHandlersAreAdded ();
		...
		public virtual object GetCommandParameter (string commandName, object parameter = null)
		{
			return parameter;
		}
