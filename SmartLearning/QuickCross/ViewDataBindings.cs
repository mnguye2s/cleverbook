	...
	public partial class ViewDataBindings
	{
		...
		private class DataBinding
		{
			...
			public void Command_CanExecuteChanged (object sender, EventArgs e)
			{
				...
				if (control != null)
					control.Enabled = ((RelayCommand)sender).IsEnabled;
			}
		}
		...
		{
			if (Object.ReferenceEquals (viewModel, newViewModel))
				return;
			RemoveHandlers ();
		...
		public void RemoveHandlers ()
		{
			foreach (var item in dataBindings) {
				...
				case BindingMode.TwoWay:
					RemoveTwoWayHandler (binding);
					break;
				case BindingMode.Command:
					RemoveCommandHandler (binding);
					break;
				}
			}
		}
		...
		{
			if (binding != null && binding.TableViewSource != null)
				binding.TableViewSource.RemoveHandlers ();
		}

		private void AddListHandlers (DataBinding binding)
		{
			if (binding != null && binding.TableViewSource != null)
				binding.TableViewSource.AddHandlers ();
		}
		...
		public void AddBindings (BindingParameters[] bindingsParameters)
		{
			if (bindingsParameters != null) {
				foreach (var bp in bindingsParameters) {
					...
					if (dataBindings.ContainsKey (IdName (bp.PropertyName)))
						throw new ArgumentException ("Cannot add binding because a binding already exists for the view with Id " + IdName (bp.PropertyName));
					AddBinding (bp); 
				}
			}
		}

		private string IdName (string name)
		{
						...
						if (cc != null) {
							...
							for (int i = 0; i < cc.Count; i++) {
								...
								if (assignmentElements.Length == 1) {
									...
									switch (name) {
									case "Mode":
										Enum.TryParse<BindingMode> (value, true, out bp.Mode);
										break;
									case "ItemsSource":
										bp.ListPropertyName = value;
										break;
									case "ItemTemplate":
										bp.ListItemTemplateName = value;
										break;
									case "AddCommand":
										bp.ListAddItemCommandName = value;
										break;
									case "RemoveCommand":
										bp.ListRemoveItemCommandName = value;
										break;
									case "CanEdit":
										bp.ListCanEditItem = value;
										break;
									case "CanMove":
										bp.ListCanMoveItem = value;
										break;
									default:
										throw new ArgumentException ("Unknown tag binding parameter: " + name);
									}
								}
							}
						}
		...
		private void UpdateView (DataBinding binding)
		{
			if ((binding.Mode == BindingMode.OneWay) || (binding.Mode == BindingMode.TwoWay) && binding.View != null) {
				...
				if (rootViewExtensionPoints != null)
					rootViewExtensionPoints.UpdateView (view, value);
				else
		...
		{
			if (rootViewExtensionPoints != null)
				parameter = rootViewExtensionPoints.GetCommandParameter (binding.ViewModelPropertyInfo.Name, parameter);
