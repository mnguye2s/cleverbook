...
namespace QuickCross
{
	public class DataBindableUITableViewSource : UITableViewSource
	{
		private class ItemDataBinding
		{
			...
			{ 
				if (ObjectPropertyInfo != null)
					return ObjectPropertyInfo.GetValue (item);
				if (ObjectFieldInfo != null)
					return ObjectFieldInfo.GetValue (item);
				return item;
			}
		...
		{ 
			AddListHandler ();
			foreach (var viewDataBindingsHolder in viewDataBindingsHolders) {
				viewDataBindingsHolder.Value.AddHandlers ();
			}
		}
		...
		{ 
			RemoveListHandler ();
			foreach (var viewDataBindingsHolder in viewDataBindingsHolders) {
				viewDataBindingsHolder.Value.RemoveHandlers ();
			}
		}
		...
		{
			if (Object.ReferenceEquals (this.list, list))
				return false;
			RemoveListHandler ();
			this.list = list;
		...
		private void EnsureItemDataBindingsHolder (UITableViewCell rootView, object itemObject)
		{
			...
			foreach (var idb in holder)
				UpdateView (idb.View, idb.GetValue (itemObject));
		}
		...
		private bool ExecuteCommand (string commandName, object parameter = null)
		{
			if (viewModel == null || string.IsNullOrEmpty (commandName))
				return false;
			if (viewExtensionPoints != null)
				parameter = viewExtensionPoints.GetCommandParameter (commandName, parameter);
		...
		protected bool GetItemFlag (NSIndexPath indexPath, string flag)
		{
			...
			if (string.IsNullOrEmpty (flag) || flag.ToLower () == "false")
				return false;
			...
			if (itemObject != null) {
				...
				if (pi != null)
					return (bool)pi.GetValue (itemObject);
				var fi = itemType.GetField (flag);
				if (fi != null)
					return (bool)fi.GetValue (itemObject);
			}
			return false;
		}
		...
		public override void MoveRow (UITableView tableView, NSIndexPath sourceIndexPath, NSIndexPath destinationIndexPath)
		{
			...
			if (itemObject == null || deleteAt == insertAt)
				return;
		...
		public override void RowSelected (UITableView tableView, NSIndexPath indexPath)
		{
			if (viewModel != null && rowSelectedPropertyName != null) {
				...
				//Tuan- show checkmark if cell is being selected and none if deselected
//				if (currentSelectedCell != null)
//					currentSelectedCell.Accessory = UITableViewCellAccessory.None;
